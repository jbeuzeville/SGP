...
@Controller
@Scope("session")
public class GestionarActividadController extends BaseController {

	private static final long serialVersionUID = 1L;

	private static final Logger MENSAJELOG = Logger
			.getLogger(GestionarActividadController.class);
	...
	private CampoGenerico filtroActividad;
	...
	@PostConstruct
	private void init() {
		MENSAJELOG.info("INI GestionarActividadController");
		this.actividad = new Actividad();
		this.listActividad = new ArrayList<Actividad>();
		this.filtroActividad = new CampoGenerico();
	...
	public void listarActividad() {
		try {
			...
			if (!UtilWeb.esNuloOrVacio(this.filtroActividad.getIdRecurso())) {
				actividad.getRecurso().setIdUsuario(
						Long.parseLong(this.filtroActividad.getIdRecurso()));
			} else {
				actividad.getRecurso().setIdUsuario(null);
			}
			if (!UtilWeb.esNuloOrVacio(this.filtroActividad
					.getIdProyectoHitss())) {
				actividad.setIdProyectoHitss(Long
						.parseLong(this.filtroActividad.getIdProyectoHitss()));
			} else {
				actividad.setIdProyectoHitss(null);
			}
			if (!UtilWeb.esNuloOrVacio(this.filtroActividad.getFechaInicio())
					&& UtilWeb
							.esNuloOrVacio(this.filtroActividad.getFechaFin())) {
				mensajeError("Por favor, seleccionar Fecha Fin.");
				return;
			}
			if (!UtilWeb.esNuloOrVacio(this.filtroActividad.getFechaFin())
					&& UtilWeb.esNuloOrVacio(this.filtroActividad
							.getFechaInicio())) {
				mensajeError("Por favor, seleccionar Fecha Inicio.");
				return;
			}
			actividad.setFecha(this.filtroActividad.getFechaInicio());
			actividad.setFechaFin(this.filtroActividad.getFechaFin());
			this.listActividad = this.actividadService
					.listarActividadAdministrador(actividad);
		} catch (Exception e) {
			String[] errores = MessageException.getMessageExceptionPrintAop(e,
					e.getMessage());
			MENSAJELOG.error(errores[ConstantesWeb.N1]);
	...
	public CampoGenerico getFiltroActividad() {
		return filtroActividad;
	}

	public void setFiltroActividad(CampoGenerico filtroActividad) {
